 ...
 <div class="add-edit-product-wrap col-12">
    ...
    <div class="add-edit-product-form">
        <form method="POST" action="{{ route('productTypes.store') }}">
            <h4 class="title">Edit ProductType</h4>
            @csrf
            <div class="row">
                ...
                <div class="col-lg-6 col-12 mb-30">
                    <input name="icon" class="form-control @error('icon') border border-danger @enderror" type="text"
                        value="{{ old('icon', isset($productType) ? $productType->icon : '') }}"
                        placeholder="ProductType Icon">
                    @error('icon')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12 mb-30">
                    <textarea name="description" class="form-control @error('description') border border-danger @enderror" placeholder="ProductType Description*">{{ $productType->description ? $productType->description : ''}}</textarea>
                    @error('description')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
